...
public class MealTool {
    ...
    public static todayBapData getTodayBap(Context mContext) {
        ...
        restoreBapDateClass mData = MealTool.restoreBapData(mContext, year, month, day);
        ...
        if (!mData.isBlankDay) {
             ...
             */
            if (hour <= 14) {
                mReturnData.title = "중식";
                mReturnData.info = (!MealLibrary.isMealCheck(mData.Lunch) ? "없는데요" : mData.Lunch);
            } else {
                mReturnData.title = "석식";
                mReturnData.info = (!MealLibrary.isMealCheck(mData.Dinner) ? "없는데요" : mData.Dinner);
            }
        } else {
            mReturnData.title = "오류";
            mReturnData.info = "오류";
        }
